...
function generateContentBulletsHTML(slide, htmlDir, assetsDir, constants) {
  ...
  // Detect AI Modules section - if we find "AI Modules:" label, render following items as bullet list
  let aiModulesIndex = -1;
  for (let i = 0; i < content.length; i++) {
    const text = content[i].text || '';
    if (text.toLowerCase().includes('ai modules') && text.includes(':')) {
      aiModulesIndex = i;
      break;
    }
  }
  ...
  content.forEach((item, index) => {
    ...
    // Special handling for AI Modules section: render as bullet list
    const isAIModulesLabel = aiModulesIndex >= 0 && index === aiModulesIndex;
    const isAIModuleItem = aiModulesIndex >= 0 && index > aiModulesIndex && level === 1;

    if (isAIModulesLabel) {
      // Render "AI Modules:" as label only
      const key = text.substring(0, colonIndex).trim();
      formattedText = `<span style="color: ${ACCENT_COLOR}; font-weight: bold;">${escapeHtml(key)}:</span>`;
      listHTML += `<li style="margin-left: 0pt; margin-bottom: 8pt; font-size: 12pt; color: ${TEXT_COLOR}; line-height: 1.4;">
        ${formattedText}
      </li>`;
      return; // Skip normal processing for this item
    } else if (isAIModuleItem) {
      // Render AI module items as individual bullets (extract module names)
      const moduleText = text.replace(/^\d+\.\s*/, '').replace(/\|.*$/, '').trim();
      formattedText = `<span style="position: absolute; left: 0; color: ${ACCENT_COLOR}; font-weight: bold;">•</span><span style="margin-left: 5pt;">${escapeHtml(moduleText)}</span>`;
      listHTML += `<li style="margin-left: 15pt; margin-bottom: 4pt; font-size: 11pt; color: ${TEXT_COLOR}; line-height: 1.4; position: relative;">
        ${formattedText}
      </li>`;
      return; // Skip normal processing for AI module items
    }

    if (colonIndex > 0) {
      const key = text.substring(0, colonIndex).trim();
      const value = text.substring(colonIndex + 1).trim();
      // For AI Modules field, skip the value (will be rendered as bullets above)
      if (key.toLowerCase().includes('ai modules')) {
        return;
      }
