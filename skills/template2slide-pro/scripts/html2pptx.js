...
// Dependency check - ensure all required modules are available
const SCRIPT_DIR = __dirname;
const NODE_MODULES_DIR = path.join(SCRIPT_DIR, 'node_modules');
...
function checkDependencies() {
  ...
  const inprocessFile = path.join(playwrightCoreLib, 'inprocess.js');

  if (!fs.existsSync(playwrightCoreLib) || !fs.existsSync(inprocessFile)) {
    throw new Error(
      'Playwright installation incomplete!\n' +
      'Run: cd ' + SCRIPT_DIR + ' && npm install\n' +
      'Or run: node ' + path.join(SCRIPT_DIR, 'check_dependencies.js') + ' --fix'
    );
  }
}
