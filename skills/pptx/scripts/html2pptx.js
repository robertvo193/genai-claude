...
function validateTextBoxPosition(slideData, bodyDimensions) {
  ...
  // SMART OVERFLOW DETECTION: Check if text box bounding box > 0.9 * page size
  ...
  for (const el of slideData.elements) {
    ...
    if (['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'list'].includes(el.type)) {
      ...
      // CHECK 1: Text box covers too much of the page (likely overflow)
      if (textBoxArea > maxArea) {
        ...
        const textPrefix = getText().substring(0, 30) + (getText().length > 30 ? '...' : '');

        errors.push(
          `⚠️ Text box "${textPrefix}" covers ${coveragePercent.toFixed(1)}% of slide (>90% limit). ` +
          `Solution: Use two-column layout or split into 2 slides. ` +
          `Current size: ${el.position.w.toFixed(2)}" × ${el.position.h.toFixed(2)}"`
        );
      }

      // CHECK 2: Text box too close to bottom edge (legacy check)
