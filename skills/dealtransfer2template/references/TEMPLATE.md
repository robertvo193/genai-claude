...
| Work Scope | S2 - "deployment method?" + S1 - "Pain Points" | Format: `[Deployment method] AI system to [general objective]` (one sentence, no module list). Keep concise and compact. |
...
| AI Modules | S2 - "List of VA use cases" | List all modules (see Logic below). **DO NOT merge modules** - list each module separately. |
...
- **DO NOT merge AI modules** - Each module from STANDARD_MODULES.md must be listed separately
  ...
  - Only group modules if STANDARD_MODULES.md has them as a single entry
...
- **List each module separately** - do NOT merge or group (e.g., separate entries for Safety Helmet, Safety Mask, Safety Vest)

**Work Scope Examples** (keep concise - one short sentence):
- "Cloud-based AI system to monitor workplace safety compliance in real time"
- "On-premise AI system to enhance operational efficiency and safety"
- "Hybrid AI system to optimize production processes and ensure worker safety"
- "Edge AI system to detect safety violations and prevent accidents"
...
**For STANDARD Modules - Extract ALL fields from `STANDARD_MODULES.md`:**

| Field | Source | Logic |
|-------|--------|-------|
| Module Name | S2 - "List of VA use cases" | Use exact module name from STANDARD_MODULES.md |
| Module Type | Check STANDARD_MODULES.md | Standard or Custom |
| Purpose Description | STANDARD_MODULES.md (Standard) or S2 (Custom) | **Copy exact text from STANDARD_MODULES.md for standard modules** |
| Alert Trigger Logic | STANDARD_MODULES.md (Standard) or S2 (Custom) | **Copy exact text from STANDARD_MODULES.md for standard modules** |
| Detection Criteria | STANDARD_MODULES.md (Standard) or S2 (Custom) | **Copy exact text from STANDARD_MODULES.md for standard modules** |
| Preconditions | STANDARD_MODULES.md (Standard) or S2 (Custom) | **Copy exact text from STANDARD_MODULES.md for standard modules** |
| Image URL | STANDARD_MODULES.md (Standard only) | **Use exact URL from STANDARD_MODULES.md, or "" if empty** |
| Video URL | STANDARD_MODULES.md (Standard only) | **Use exact URL from STANDARD_MODULES.md, or "" if not available** |
| Client Data Requirements | S2 - "customized AI use cases" (Custom only) | Training data needed (if custom) |

**IMPORTANT for Standard Modules:**
- Extract ALL fields directly from STANDARD_MODULES.md
- Do NOT paraphrase or summarize - use exact text
- For Image/Video URLs: Use the exact URL from the table, or "" if the cell is empty
...
**IMPORTANT: Include this section ONLY if custom requirements exist. Skip entirely if no custom requirements.**

### When to INCLUDE Section 8:
...
- S2 - "How do they want to alert operators on-site?" mentions non-standard channels (VMS, on-site alarms, beyond Dashboard/Email/Telegram/SMS)

### When to SKIP Section 8:

**SKIP if ALL of the following apply:**
- S2 - "Any customized dashboard?" = "no" or blank
- S2 - "How do they want to alert operators on-site?" only mentions standard channels (Dashboard, Email, Telegram, SMS, Mobile App)
