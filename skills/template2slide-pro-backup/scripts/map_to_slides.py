...
class SlideMapper:
    ...
    def _extract_modules(self, content: str) -> List[Dict[str, Any]]:
        ...
        # Pattern 1: ### Module [number]: [Name] or #### Module [number]: [Name] (markdown header format - 3 or 4 hashes)
        # Supports both "### Module: Name" and "### Module 1: Name" and "#### Module 1: Name"
        pattern1 = r'^#{3,4}\s+Module(?:\s+\d+)?\s*:\s*(.+?)$'
        matches1 = list(re.finditer(pattern1, content, re.IGNORECASE | re.MULTILINE))
        ...
        for match in matches1:
            ...
            next_module = re.search(r'^#{3,4}\s+Module(?:\s+\d+)?\s*:\s*', content[start_pos:], re.IGNORECASE | re.MULTILINE)
