 ...
 *
 * Note: This script is typically called via the `run-pptx` alias which sets up
 * the required environment (Python venv, Puppeteer config) before execution.
 */
...
async function renderMermaidDiagram(mermaidCode, outputPath) {
  try {
    ...
    try {
      ...
      const puppeteerConfig = process.env.PUPPETEER_CONFIG || '';
      const puppeteerConfigFlag = puppeteerConfig ? `-p "${puppeteerConfig}"` : '';
      execSync(`mmdc -i "${tempMmdFile}" -o "${outputPath}" -b transparent -t dark ${puppeteerConfigFlag}`, {
        stdio: 'inherit'
      });
...
async function generatePresentation(inputJsonPath) {
  ...
  // Create content-only backup before reference slides are inserted
  try {
    const contentOnlyBackup = outputPptx.replace('.pptx', '_content_only.pptx');
    const fs = require('fs');
    fs.copyFileSync(outputPptx, contentOnlyBackup);
    console.log(`\n✓ Created content-only backup: ${path.basename(contentOnlyBackup)}`);
  } catch (error) {
    console.error(`Error creating content-only backup: ${error.message}`);
  }
...
if (require.main === module) {
  ...
  if (!inputJsonPath) {
    console.error('Usage: run-pptx <input_json_file>');
    process.exit(1);
  }
